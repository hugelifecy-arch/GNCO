...
if (
  !/<script\s+type=["']module["'][^>]*src=["'](?:\/GNCO)?\/assets\/[^"']+["']/.test(
    indexHtml
  )
) {
  fail(
    `dist/index.html MUST contain a module <script> with src="/GNCO/assets/..." (build output)`
  );
}

const artifacts = collectFiles(deploy.dir);
const htmlArtifacts = artifacts.filter((p) =>
